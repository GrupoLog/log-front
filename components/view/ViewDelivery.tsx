...
import { getDeliveryById, getDriverById, getTripById, getVehicleById } from "@/services/APIService"
import { Separator } from "@/components/ui/separator"

export function ViewDelivery({ entrega }) {
    ...
    async function fetchDelivery(idEntrega) {
        try {
            const data = await getDeliveryById(idEntrega)
            setDelivery(data)
    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => {
            setOpen(isOpen)
            if (isOpen && entrega.id_viagem) {
                fetchDelivery(entrega.id_viagem)
                // if (viagem.chassi) {
