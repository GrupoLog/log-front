...
interface EditModalProps {
  ...
  onClose: () => void;
  client: Client;
  onUpdatedClient: (updated: Client) => void;
}

const ClientEditModal: React.FC<EditModalProps> = ({
  isOpen,
  onClose,
  client,
  onUpdatedClient,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/clientes/${client.cpf}`,
        {
          method: "PUT",
      ...
      if (!response.ok) throw new Error("Erro response ao atualizar cliente.");

      const updatedClient = await response.json();
      onUpdatedClient(updatedClient);
      onClose();
    } catch (error) {
      console.error(error);
